		...
		for (unsigned int i = 0; i < Aliases.size(); i++)
		{
			if (Aliases[i].text == c)
			{
				...
				if ((u != NULL) && (!Aliases[i].requires.empty()) && (Aliases[i].uline))
				{
					...
					{
						ServerInstance->WriteOpers("*** Service "+Aliases[i].requires+" required by alias "+std::string(Aliases[i].text.c_str())+" is not on a u-lined server, possibly underhanded antics detected!"); 
